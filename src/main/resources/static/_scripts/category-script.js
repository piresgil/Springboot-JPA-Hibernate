...
*/
    async function carregarDadosDeCategorias() {
        const rawJsonContainer = document.getElementById('raw-json-container');
        const formattedDataContainer = document.getElementById('category-data-container');

        try {
            // AQUI VOCÊ FAZ A REQUISIÇÃO REAL PARA SEU BACKEND JAVA
            const response = await fetch('/categories'); // OU a URL completa 'https://seu-backend.onrender.com/api/utilizadores'
            ...
            if (realJsonData && realJsonData.length > 0) {
                let htmlTable = '<table>';
                htmlTable += '<thead><tr><th>ID</th><th>Nome</th></thead>';
                htmlTable += '<tbody>';

                realJsonData.forEach(category => {
                ...
                formattedDataContainer.innerHTML = '<p>Nenhuma Categoria encontrada.</p>';
            }
    ...
    window.onload = carregarDadosDeCategorias;
